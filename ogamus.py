...
def main(controller):
    ...
    args_parser.add_argument(
        "-t",
        "--task",
        help="Task name,  e.g. on, close, open or ogn",
        type=str,
        default="open",
    )
    args_parser.add_argument(
        "-obj",
        "--object_ground_truth",
        help="Use ground truth object detections",
        default=None,
    ...
    if task is not None:
        assert task in [
            Configuration.TASK_ON,
            Configuration.TASK_OGN_ROBOTHOR,
            Configuration.TASK_OPEN,
            Configuration.TASK_CLOSE,
            Configuration.TASK_OGN_ITHOR,
            Configuration.TASK_OGN_ROBOTHOR,
    ...
    dataset = json.load(
        open(
            os.path.join(
                Configuration.DATASET_DIR, "{}.json".format(Configuration.DATASET)
            ),
            "r",
        )
    )
    ...
    if Configuration.TASK == Configuration.TASK_ON:
        shutil.copyfile(
            "OGAMUS/Plan/PDDL/domain_on.pddl", "OGAMUS/Plan/PDDL/domain.pddl"
        )
    if (
        ...
        shutil.copyfile(
            "OGAMUS/Plan/PDDL/domain_open_close.pddl", "OGAMUS/Plan/PDDL/domain.pddl"
        )
    if (
        ...
        shutil.copyfile(
            "OGAMUS/Plan/PDDL/domain_ogn.pddl", "OGAMUS/Plan/PDDL/domain.pddl"
        )
        ...
        if episode in list(range(starting_episode, 5000)):
            ...
            logger.write(
                ...
                "###############################################\n".format(
                    ...
                    Configuration.GOAL_OBJECTS,
                    Configuration.MAX_ITER,
                    Configuration.VISIBILITY_DISTANCE,
                    Configuration.MOVE_STEP,
                    Configuration.TASK == Configuration.TASK_OGN_ROBOTHOR,
                    Configuration.ROTATION_STEP,
                    Configuration.FOV,
                    Configuration.MAX_DISTANCE_MANIPULATION,
                    Configuration.IOU_THRSH,
                    Configuration.GROUND_TRUTH_OBJS,
                    Configuration.OPEN_CLASSIFIER_THRSH,
                    Configuration.OPEN_CLASSIFIER_THRSH,
                    Configuration.OBJ_DETECTOR_PATH,
                    Configuration.OPEN_CLASSIFIER_PATH,
                    Configuration.ON_CLASSIFIER_PATH,
                )
            )
