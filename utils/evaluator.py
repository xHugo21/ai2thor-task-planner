...
class Evaluator:
    def __init__(self):
        ...
        self.obj_classes = [
            obj_class.lower()
            for obj_class in pickle.load(open(Configuration.OBJ_CLASSES_PATH, "rb"))
        ]
        ...
        for belief_obj in belief_objects:
            ...
            if belief_obj["id"] not in self.objs_id_mapping.keys():
                ...
                [
                    ...
                    and obj["objectType"].lower()
                    not in Configuration.IRRELEVANT_CLASSES
                ...
                for obj in visible_gt:
                    ...
                    if (
                        ...
                        and gt_bbox["y2"] > gt_bbox["y1"]
                        and self.get_iou(belief_bbox, gt_bbox)
                        > Configuration.OBJ_IOU_MATCH_THRSH
                    ):
                        self.objs_id_mapping[belief_obj["id"]] = obj
                        break

                if (
                    belief_obj["id"] not in self.objs_id_mapping
                    and Configuration.GROUND_TRUTH_OBJS
                ):
                    logger.write(
                        "Warning: cannot map detected object {} into a ground truth one, even if "
    ...
    def evaluate_state(self, agent):
        ...
        belief_objects = []
        [
            belief_objects.extend(v)
            for k, v in agent.learner.knowledge_manager.all_objects.items()
        ]
        belief_objects = [obj["id"] for obj in belief_objects]
        mapped_belief_objects = [
            self.objs_id_mapping[obj_id]
            if obj_id in self.objs_id_mapping
            else "unknown_" + obj_id
        ...
        for pred in belief_predicates:
            for belief_obj_id in belief_objects:
                if belief_obj_id in self.objs_id_mapping:
                    pred = pred.replace(
                        belief_obj_id + ")", self.objs_id_mapping[belief_obj_id] + ")"
                    )
                    pred = pred.replace(
                        belief_obj_id + ",", self.objs_id_mapping[belief_obj_id] + ","
                    )
        ...
        mapped_belief_predicates_global = [
            pred
            for pred in mapped_belief_predicates
        ...
        metrics["predicates"]["fn"] = len(
            [
                pred
                for pred in metrics["predicates"]["gt"]
        ...
        metrics["objects"]["tp"] = len(
            [
                obj
                for obj in metrics["objects"]["belief"]
                if obj in metrics["objects"]["gt"]
            ]
        )
        metrics["objects"]["fp"] = len(
            [
                obj
                for obj in metrics["objects"]["belief"]
        ...
        metrics["objects"]["fn"] = len(
            [
                obj
                for obj in metrics["objects"]["gt"]
        ...
        metrics["objects"]["belief_counting"] = (
            agent.learner.knowledge_manager.objects_counting
        )
        metrics["objects"]["belief_avg_scores"] = {
            k: str(v)
            for k, v in agent.learner.knowledge_manager.objects_avg_score.items()
        }
        ...
        metrics["global"]["tp"] = len(
            [
                pred
                for pred in mapped_belief_predicates_global
                if pred in metrics["predicates"]["gt"]
            ]
        )
        metrics["global"]["fp"] = len(
            [
                pred
                for pred in mapped_belief_predicates_global
                if pred not in metrics["predicates"]["gt"]
    ...
    def update_gt_state(self, visible_objects, last_event):
        ...
        for obj_id in self.gt_objs_seen:
            ...
            agent_pos = [
                last_event.metadata["agent"]["position"]["x"],
            ...
            for pred in self.gt_predicates:
                if (
                    pred.split("(")[0].lower() == "inspected"
                    and pred.split("(")[1].strip()[:-1].lower() == obj_id
                ):
                    obj_inspected = True
            ...
            if obj["parentReceptacles"] is not None:
                for container_name in obj["parentReceptacles"]:
                    ...
                    if (
                        not container_name.split("|")[0].lower().strip()
                        in Configuration.IRRELEVANT_CLASSES
                    ):
                        containers.append(container_name)

            gt_predicates[obj_id] = {
                "holding": obj_picked,
                "open": obj_open,
                "discovered": True,
                "viewing": obj_visible,
        ...
        gt_predicates_list = [
            "{}({})".format(k2, k)
            for k, v in gt_predicates.items()
            for k2, v2 in v.items()
            if type(v2) == type(True) and v2
        ]
        if hand_free:
            gt_predicates_list.append("hand_free()")

        # Get binary predicates
        gt_predicates_list.extend(
            [
                "{}({},{})".format(k2, k, v3)
                for k, v in gt_predicates.items()
                for k2, v2 in v.items()
    ...
    def eval_goal_achievement_objectnav(
        self, all_objects, last_event, agent_path, controller
    ):
        assert self.shortest_path is not None, (
            "Set shortest_path in evaluator.py for computing SPL for "
        ...
        goal_objects = [
            el for el in goal_objects.strip()[1:-1].split() if el.strip() != "-"
        ]
        goal_objects_id = {
            goal_objects[i]: goal_objects[i + 1]
        ...
        objects_visibles_and_near_to_agent = [
            o["objectType"].lower()
            for o in last_event.metadata["objects"]
            if o["visible"]
        ]
        ...
        if goal_object_type in objects_visibles_and_near_to_agent:
            ...
            goal_objs = [
                ...
                if goal_object_type.lower() == o["objectType"].lower()
            ]

            for obj in goal_objs:
                if (
                    Configuration.TASK == Configuration.TASK_OGN_ROBOTHOR
                    and obj["distance"] < 1
                ):
                    success = 1
                    break
                elif (
                    Configuration.TASK == Configuration.TASK_OGN_ITHOR
                    and obj["distance"] < 1.5
                ):
                    success = 1
                    break
        ...
        uppercase_goal_obj_type = [
            o["objectType"]
            for o in last_event.metadata["objects"]
            if o["objectType"].lower() == goal_object_type
        ][0]

        try:
            dts_distance = path_distance(
                get_shortest_path_to_object_type(
                    controller,
                    uppercase_goal_obj_type,
            ...
            logger.write(
                "WARNING: computing distance to success as the distance from nearest goal object instance of the goal class,"
            ...
            dts_distance = max(
                0, dts_distance - 1
            )  # Remove the maximum distance from the object to succeeds
        ...
        metrics = json.load(
            open(
                "{}/metrics.json".format(logger.LOG_DIR_PATH),
            )
        )
        ...
        logger.write(
            "Average metrics over {} episodes:\n\t AVG success: {}\n\t AVG SPL: {}\n\t AVG dts: {}".format(
                episodes,
                sum(all_success) / episodes,
                sum(all_spl) / episodes,
                sum(all_dts) / episodes,
            )
        )
    ...
    def eval_goal_achievement_open(self, all_objects, last_event):
        ...
        goal_predicate_type = [
            goal_facts[i].strip()[1:-1].split()[0]
            for i in range(len(goal_facts))
            if not "inspected" in goal_facts[i].strip()[1:-1].split()[0]
            and not "manipulated" in goal_facts[i].strip()[1:-1].split()[0]
        ][0]
        ...
        success = 0
        for sat_goal_fact in [
            el for el in self.gt_predicates if el.startswith(goal_predicate_type)
        ]:
            if (
                bool(
                    np.all(
                        [
                            "(" + obj + "|" in sat_goal_fact.lower()
                            or "," + obj + "|" in sat_goal_fact.lower()
                            or "|" + obj + ")" in sat_goal_fact.lower()
                            for obj in goal_objects_types
                        ]
                    )
                )
                is True
        ...
        if success:
            ...
            if Configuration.TASK == Configuration.TASK_ON:
                ...
                contained_instances = [
                    o
                    for o in last_event.metadata["objects"]
                    if o["objectType"].lower() == contained_type.lower()
                ]
                container_instances = [
                    o
                    for o in last_event.metadata["objects"]
                    if o["objectType"].lower() == container_type.lower()
                ]
                dts_distance = 10000
                for contained_obj_inst in contained_instances:
                    for container_obj_inst in container_instances:
                        contained_center_3D = np.array(
                            [
                                contained_obj_inst["axisAlignedBoundingBox"]["center"][
                                    "x"
                                ],
                                contained_obj_inst["axisAlignedBoundingBox"]["center"][
                                    "y"
                        ...
                        container_center_3D = np.array(
                            [
                                container_obj_inst["axisAlignedBoundingBox"]["center"][
                                    "x"
                                ],
                                container_obj_inst["axisAlignedBoundingBox"]["center"][
                                    "y"
                        ...
                        distance = np.linalg.norm(
                            contained_center_3D - container_center_3D
                        )
                ...
                Configuration.TASK == Configuration.TASK_OPEN
                or Configuration.TASK == Configuration.TASK_CLOSE
            ):
                goal_obj_type = goal_objects_types[0]
                goal_obj_instances = [
                    o
                    for o in last_event.metadata["objects"]
                    if o["objectType"].lower() == goal_obj_type.lower()
                ]
                dts_distance = 10000
                for goal_obj_inst in goal_obj_instances:
                    agent_center = np.array(
                        [
                            last_event.metadata["agent"]["position"]["x"],
                            last_event.metadata["agent"]["position"]["z"],
                        ]
                    )
                    goal_obj_center = np.array(
                        [
                            goal_obj_inst["axisAlignedBoundingBox"]["center"]["x"],
